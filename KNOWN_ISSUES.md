...
### Portuguese accents mangled in TTS pipeline (OPEN)
- **Symptom**: Brazilian Portuguese voices mispronounce accented words (opção, nós, código). The accent characters are lost or transformed somewhere in the pipeline.
- **Investigated**: `clean_for_tts.py` preserves accents correctly. `json.dumps()` escapes them to `\u00XX` by default. Adding `ensure_ascii=False` did not fix the issue — root cause is deeper in the pipeline.
- **Status**: Needs proper investigation with tests. See test suite in `tests/` for accent preservation coverage.
